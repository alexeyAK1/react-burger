export { default as BurgerFactoryPage } from "./burger-factory-page/burger-factory-page";
export { default as FeedPage } from "./feed-page/feed-page";
export { default as ForgotPasswordPage } from "./forgot-password-page/forgot-password-page";
export { default as IngredientPage } from "./ingredient-page/ingredient-page";
export { default as LoginPage } from "./login-page/login-page";
export { default as NotFound404Page } from "./not-found-404-page/not-found-404-page";
export { default as ProfilePage } from "./profile-page/profile-page";
export { default as RegisterPage } from "./register-page/register-page";
export { default as ResetPasswordPage } from "./reset-password-page/reset-password-page";

